      ...
      try {
        ...
        if (savedAreaNo) {
          const space = response.data.find(
            (s) => s.area_no === parseInt(savedAreaNo)
          );
          if (space)
            await handleSpaceSelect(space, savedRoomNo, savedStorageNo);
        }
      } catch (error) {
      ...
      <div className="home-content" style={{ backgroundColor: "#ffffff" }}>
        ...
        <div className="dropdown-buttons" style={{ marginBottom: "150px" }}>
          ...
          <div className="dropdown">
            <button
              className="dropdown-button"
              onClick={handleStorageDropdownClick}
            >
            ...
            <ul className="drawer-wrapper">
              {[...Array(drawerRows)].map((_, index) => (
                <li
                  ...
                  key={index}
                  style={{ "--i": drawerRows - index }}
                >
                  <button
                    className={`drawer-button ${
                      selectedRow === drawerRows - index ? "active" : ""
                    }`}
